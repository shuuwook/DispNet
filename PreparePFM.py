...
def read(file):
    ...
    elif file.endswith('.pfm'): return readPFM(file, True)[0]
...
def readPFM(file, crop):
    if crop:
        if not os.path.isfile(file + '.H.pfm'):
            x, scale = readPFM(file, False)
            x_ = np.zeros((384, 768), dtype=np.float32)
            for i in range(77, 461):
                for j in range(96, 864):
                    x_[i - 77, j - 96] = x[i, j]
            writePFM(file + '.H.pfm', x_, scale)
            return x_, scale
        else:
            file += '.H.pfm'
...
def readFlow(name):
    if name.endswith('.pfm') or name.endswith('.PFM'):
        return readPFM(name, False)[0][:,:,0:2]
